...
get : Session -> Data.Id.Id -> Task ( Session, Http.Error ) Data.Playlist.Playlist
get session id =
    Http.task
        { method = "GET"
        , headers = [ Api.authHeader session ]
        , url = Api.url ++ "playlists/" ++ Data.Id.idToString id ++ "?fields=description,uri,id,images,name,uri,owner"
        , body = Http.emptyBody
...
getTracks : Session -> Data.Id.Id -> Int -> Task ( Session, Http.Error ) Data.Track.PlaylistTrackObject
getTracks session id offset =
    Http.task
        { method = "GET"
        , headers = [ Api.authHeader session ]
        , url = Api.url ++ "playlists/" ++ Data.Id.idToString id ++ "/tracks?&offset=" ++ String.fromInt offset ++ "&limit=100"
        , body = Http.emptyBody
